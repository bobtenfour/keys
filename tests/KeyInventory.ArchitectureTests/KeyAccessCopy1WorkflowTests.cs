...
public sealed class KeyAccessCopy1WorkflowTests : IAsyncLifetime
{
    ...
    public async Task ManyCopiesPerKeyNumberWithMedecoUniquenessAndCrossKeyMedecoReuse()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
                createKey.ExecuteAsync("66800", "26", "mechanical", CancellationToken.None))
            .ConfigureAwait(true);

        await createKey.ExecuteAsync("66801", "26", "mechanical", CancellationToken.None).ConfigureAwait(true);
    ...
    [Fact]
    public async Task KeyNumberRoomManyToManyAndIdenticalDerivedRoomsForEveryCopy()
    {
        using IServiceScope scope = CreateScope();
        ICreateKeyAssetUseCase createKey = scope.ServiceProvider.GetRequiredService<ICreateKeyAssetUseCase>();
        ...
        IKeyAccessPatternRoomAssignmentPersistencePort assignmentPort =
            scope.ServiceProvider.GetRequiredService<IKeyAccessPatternRoomAssignmentPersistencePort>();
        ...
        await assignments.AssignRoomAsync("66800", roomA, CancellationToken.None).ConfigureAwait(true);
        await assignments.AssignRoomAsync("66800", roomB, CancellationToken.None).ConfigureAwait(true);
        await assignments.AssignRoomAsync("66801", roomA, CancellationToken.None).ConfigureAwait(true);

        IReadOnlyList<string> forRoomA = await assignmentPort
            .ListKeyNumbersForRoomAsync(roomA, CancellationToken.None)
            .ConfigureAwait(true);
        Assert.Contains("66800", forRoomA);
        Assert.Contains("66801", forRoomA);

        IReadOnlyList<KeyOpenedRoomItem> patternRooms = await assignments
            .ListOpenedRoomsAsync("66800", CancellationToken.None)
            .ConfigureAwait(true);
        Assert.Equal(2, patternRooms.Count);
        ...
        {
            Assert.Equal(2, copy.OpenedRooms.Count);
            Assert.Equal(
    ...
    public async Task FindAndReportsDistinguishKeyNumberAndMedeco()
    {
        ...
        IReadOnlyList<string> keyNumbersForRoom = await assignmentPort
            .ListKeyNumbersForRoomAsync(roomCode, CancellationToken.None)
            .ConfigureAwait(true);
        ...
        Assert.Contains(catalog, row => row.KeyNumber == "66800" && row.MedecoKeyCode == "26");
        Assert.Contains(catalog, row => row.KeyNumber == "66800" && row.MedecoKeyCode == "27");

        string catalogCsv = reports.FormatKeyCatalogCsv(catalog);
        Assert.Contains("KEY #", catalogCsv, StringComparison.Ordinal);
        Assert.Contains("MEDECO", catalogCsv, StringComparison.Ordinal);
        Assert.Contains("26", catalogCsv, StringComparison.Ordinal);
        Assert.Contains("27", catalogCsv, StringComparison.Ordinal);

        IReadOnlyList<string> keyNumbers = await reports.ListKeyNumbersAsync("668", CancellationToken.None)
            .ConfigureAwait(true);
        Assert.Contains("66800", keyNumbers);
        ...
        Assert.Contains(audits, item => item.ActionType == OperatorAuditActions.KeyAccessPatternRoomAssignmentAdded);
    ...
    public void WebDoesNotReferenceDbContextAndNormalizedOwnershipIsPreserved()
    {
        ...
        Assert.DoesNotContain(
            ...
            property => property.Name is "CatalogKeyCode" or "IntendedLock");
        Assert.Contains(
            typeof(KeyInventory.Domain.Catalog.KeyAsset).GetProperties(),
