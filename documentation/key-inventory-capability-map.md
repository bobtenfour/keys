...
- Key Catalog capability: create KEY # with Classification-defined access (Regular one Room; Master all Rooms); register MEDECO copies under existing KEY # without changing access; no independent KEY #↔Room assign/remove capability.
- Find/Search capability: reverse Room lookup returns Regular matches plus all Master KEY #s.
- Room Administration
...
- Key Catalog includes KEY # / KeyAccessPattern (shared access-pattern identity), Regular|Master Classification on KEY # (replaces KeyType), physical KeyAsset copies identified by MEDECO Key Code unique within KEY #, and current KeyAccessPattern↔Room opening assignments; physical copies derive Rooms and Classification from their KEY #; KeyAsset stores no holder/Department; Available/Issued derived from open Loan; Lock is not required as intermediate room-opening authority; KeySeries is not KEY # or Room-access authority; master/sub-master hierarchy is out of scope; Classification Master is explicit and is not inferred from Room count.
...
- Workforce Key Eligibility includes Department with immutable DepartmentId and global unique editable DepartmentCode, Room with RoomCode technical identity, global RoomNumber, and required Room→Department membership, Party as persistent person identity with UIN, WorkforceMember as the workforce relationship and eligibility authority for WorkforceType Employee and Contractor, WorkAssignment to Room (technical WorkAssignmentId; no operator WorkAssignmentCode; no Primary) with Room.Department == WM.Department, key-issue eligibility (including at least one active WorkAssignment), and termination return obligations completed through existing Return authority, without Organization, Building, ResponsibleManager, Employment aggregate, Borrower aggregate, temporary borrower fields, duplicate person-identity authority, or HR integration.
- Room Administration exists because Room is the real operational place concept for this installation and belongs to exactly one Department; it does not expand into Building, Campus, or enterprise location hierarchies.
- Department Administration exists for workforce membership, Room ownership, and Department-based issue justification without Organization ownership.
...
- Operator interaction architecture (active presentation): Global Search header→results; primary navigation Home → Catalog → Operations → remaining modules; Create Key **New Key** / **Replace Lost Key** (Application resolves existing vs new KEY #; no separate Create New KEY # mode; no Key Types admin); Issue physical-key-first searchable combobox then holder under Operations; Receive open-custody searchable combobox; Room Assignment (Assign Room) searchable combobox; no navigation-as-information-substitute on Person search results.
